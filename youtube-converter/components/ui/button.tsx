...
function Button({
  ...
  size,
  pressOpacity,
  pressOpacityDurationMs,
  ...props
}: ButtonProps) {
  const pressedOpacity = pressOpacity ?? 1;
  const pressedOpacityDuration = pressOpacityDurationMs ?? 120;

  const opacity = useSharedValue(props.disabled ? 0.5 : 1);
  const animatedStyle = useAnimatedStyle(() => {
    return { opacity: props.disabled ? 0.5 : opacity.value };
  }, [props.disabled]);
    ...
    <TextClassContext.Provider value={buttonTextVariants({ variant, size })}>
      <AnimatedPressable
        ...
        onPressIn={(e) => {
          ...
          opacity.value = withTiming(pressedOpacity, { duration: pressedOpacityDuration });
          props.onPressIn?.(e);
        }}
        onPressOut={(e) => {
          if (props.disabled) return;
          opacity.value = withTiming(1, { duration: pressedOpacityDuration });
          props.onPressOut?.(e);
        }}
        {...props}
      />
